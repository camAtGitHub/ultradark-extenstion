...
- **LOGIC:** This is the most fragile part. The algorithm must _guess_ the image's purpose.
...
My initial analysis focused on the _structure_ of the two methods. Let's now refine them with the precision required to solve this exact problem.
...
4.  _Only then_, if the theme-switching script is at the end of the `<body>` (a common practice for performance), does it execute, detect the user's preference for a dark theme, and apply the changes.

That delay between step 3 and 4 is the flash.

### The Definitive Solution: The Pre-emptive Inline Script

To solve this, we must apply the correct theme _before_ the first paint. This can only be achieved by placing a small, render-blocking script in the `<head>` of the document.
...
<!DOCTYPE html>
<html lang="en">
  <!-- The data-theme attribute will be injected here -->
  <head>
    <meta charset="UTF-8" />
    ...
    <!-- Your CSS with the theme variables -->
  </head>
  <body>
    <!-- Your page content -->
  </body>
...
- **After:** We now specify that the _initial detection_ runs in a blocking head script, while the _user-interactive part_ (e.g., a toggle button click handler) can be loaded later with a deferred script at the end of `<body>`. The state is set before the first paint, and the flash is eliminated. **This is the industry-standard, correct approach.**

#### For Algorithm 2 (The Surgeon): **Fundamentally Flawed**

This is where the Surgeon's Method truly shows its weakness.

- **The Problem:** The entire premise of this algorithm is to traverse a _rendered DOM_. It cannot run until the `<body>` exists. By then, it's too late—the flash has already happened.
