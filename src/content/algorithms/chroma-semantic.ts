...
let backgroundCache = new WeakMap<HTMLElement, string>();
...
function applySemanticStyling(
  ...
  if (hasExplicitBg || role !== "generic") {
    ...
    let bgColor: string = BACKGROUND_PALETTE[elevationLevel];
...
export function applyChromaSemantic(settings: Settings): void {
  ...
  // Initialize stats AFTER cleanup
  stats = {
    startTime,
    elementsProcessed: 0,
    variablesHijacked: 0,
...
// Expose to window for console access (using script injection + message passing for F12 console visibility)
